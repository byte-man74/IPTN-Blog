...
export default function AdminAnalytics() {
  ...
  const analyticsData = {
    summary: [
      { id: 1, title: 'Total Page Views', value: '124,582', change: '+12.3%', icon: <BarChart2 className="h-6 w-6 text-blue-500" /> },
      { id: 2, title: 'Unique Visitors', value: '45,678', change: '+8.7%', icon: <UsersIcon className="h-6 w-6 text-green-500" /> },
      { id: 3, title: 'Avg. Session Duration', value: '3m 42s', change: '+2.1%', icon: <Clock className="h-6 w-6 text-purple-500" /> },
      { id: 4, title: 'Conversion Rate', value: '3.2%', change: '+0.8%', icon: <TrendingUp className="h-6 w-6 text-orange-500" /> },
    ],
    popularPosts: [
              ...
              <div className="bg-white/60 backdrop-blur-md border border-blue-100 shadow-sm p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      <h3 className="text-sm font-medium text-gray-900">Advanced Analytics Available</h3>
                      <p className="text-sm text-gray-500">View detailed user behavior, conversion funnels, and custom event tracking</p>
                    </div>
                  </div>
                  <a
                    ...
                    {analytics.popularPosts.map((post) => (
                      <li key={post.id}>
                        <div className="px-4 py-4 sm:px-6">
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-medium text-primaryGreen truncate">{post.title}</p>
                ...
                <div className="mt-2 bg-white shadow overflow-hidden relative">
                    ...
                    {analytics.trafficSources.map((source) => (
                      <div key={source.id} className="mt-4 first:mt-0">
                        ...
                        <div className="mt-2 w-full bg-gray-200 h-2.5">
                          <div className={`${source.color} h-2.5`} style={{ width: `${source.percentage}%` }}></div>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center bg-white/70">
                    <div className="text-center">
                      ...
                      <h3 className="mt-2 text-sm font-medium text-gray-900">Detailed Traffic Analytics</h3>
                      <p className="mt-1 text-sm text-gray-500">View complete traffic source breakdown with geographic data</p>
                      <div className="mt-4">
                        <a
                          href="https://mixpanel.com"
                          target="_blank"
